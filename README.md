...
`gkLinks.html` をイントラサーバー (IIS / nginx / Apache / 共有ドライブ) に置けば、全社員が同じURLから開けます。

- **ライセンス料・月額・ユーザー数課金: 0円** — SaaS のリンク管理ツールでよくある「人数 × 月額」が発生しません
- **インストール作業不要** — 社員はブラウザで URL を開くだけ
- **データは各社員のブラウザ内 (LocalStorage)** に個別保存、外部送信なし
- **イントラ設置でオフライン動作可**
- **共通リンク集を `gkLinks_starter.json` として一発配布**、各社員は自由にカスタマイズ

> 「全員に同じスタートラインを渡す」運用が、追加コストゼロで実現します。
